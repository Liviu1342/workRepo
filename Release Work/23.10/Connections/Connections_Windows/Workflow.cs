...
namespace Connections_Windows
{
    public class Workflow : CodedWorkflow
    {
        ...
        public void Execute()
        {
            ...
            var myScreen = "Main Screen";
            var myDescriptor = "Digit 1";
            
            ClickDigitOne(myScreen, myDescriptor);

        }
        
        protected void ClickDigitOne(string screen, string descriptor)
        {            
            uiAutomation.Open(screen);
            
            uiAutomation.Attach(screen).Click(descriptor);
